        ...
        <div className="space-y-4 text-slate-600">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-red-50 p-2 rounded border border-red-100">
              <span className="font-bold text-red-600">🔥 ไฟ (D):</span>{" "}
              มุ่งมั่น, รวดเร็ว
            </div>
            <div className="bg-yellow-50 p-2 rounded border border-yellow-100">
              <span className="font-bold text-yellow-600">💨 ลม (I):</span>{" "}
              สร้างสรรค์, ช่างเจรจา
            </div>
            <div className="bg-green-50 p-2 rounded border border-green-100">
              <span className="font-bold text-green-600">⛰️ ดิน (S):</span>{" "}
              มั่นคง, ใส่ใจคน
            </div>
            <div className="bg-blue-50 p-2 rounded border border-blue-100">
              <span className="font-bold text-blue-600">💧 น้ำ (C):</span>{" "}
              ละเอียด, มีหลักการ
            </div>
          </div>
        </div>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 text-slate-600">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 font-mono text-xs md:text-sm">
            <div className="flex justify-between mb-2">
              <span>✅ เลือก มากที่สุด (Most)</span>
              <span className="text-green-600 font-bold">+1 คะแนน</span>
            </div>
            <div className="flex justify-between border-t border-slate-200 pt-2">
              <span>❌ เลือก น้อยที่สุด (Least)</span>
              <span className="text-red-500 font-bold">-1 คะแนน</span>
            </div>
          </div>
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
            ...
            {slides.map((_, i) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  i === step ? "bg-blue-600 w-4" : "bg-slate-300"
                }`}
              />
            ))}
          </div>
          ...
          <button
            ...
            disabled={step === 0}
            className="p-2 rounded-lg hover:bg-white disabled:opacity-30 disabled:hover:bg-transparent transition text-slate-600"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <button
            ...
            className="p-2 rounded-lg hover:bg-white transition text-slate-600 flex items-center gap-1 font-medium"
          >
            {step === slides.length - 1 ? (
              <span className="text-blue-600">เข้าใจแล้ว</span>
            ) : (
